...
{
    public UpdateProductRequestValidator(IReadRepository<Product> productRepo, IReadRepository<Category> brandRepo, IStringLocalizer<UpdateProductRequestValidator> T)
    {
        RuleFor(p => p.Name)
            .NotEmpty()
            .MaximumLength(75)
